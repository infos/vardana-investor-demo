...
import { DemoShell, VardanaLogo, FhirFootnote } from './DemoShell';
import { useIsMobile } from './useIsMobile';
import { DEMO_BASE, RAW_TOKEN } from '../demoPath';
  ...
  return (
    ...
    >
      <div style={{
        ...
        letterSpacing: '0.06em', color: badgeColor, background: `${badgeColor}15`,
        border: `1px solid ${badgeColor}30`, borderRadius: DT.radius.sm,
        padding: '3px 8px', marginBottom: 14, fontFamily: DT.font.body,
      }}>
        {badge}
      </div>
      <h2 style={{ fontFamily: DT.font.display, fontSize: 22, fontWeight: 400, color: DT.text.primary, margin: '0 0 8px', letterSpacing: '-0.02em' }}>
        {title}
      </h2>
      <p style={{ fontSize: 13, color: DT.text.secondary, lineHeight: 1.6, margin: '0 0 16px', fontFamily: DT.font.body }}>
        {description}
      </p>
      <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 20px' }}>
        {bullets.map((b, i) => (
          <li key={i} style={{ fontSize: 12, color: DT.text.secondary, marginBottom: 6, display: 'flex', gap: 8, lineHeight: 1.5, fontFamily: DT.font.body }}>
      ...
      <div style={{
        background: ctaBackground, color: ctaColor, borderRadius: DT.radius.md,
        padding: '11px 0', textAlign: 'center', fontSize: 13, fontWeight: 700, fontFamily: DT.font.body,
      }}>
        {ctaLabel}
      </div>
    </div>
...
export default function ClinicalDemoEntry({ navigate }) {
  ...
  return (
    <DemoShell maxWidth={680}>
      ...
      <div style={{ textAlign: 'center', marginBottom: 36 }}>
        <VardanaLogo />
        <h1 style={{
          fontFamily: DT.font.display, fontSize: 28, fontWeight: 400,
          color: DT.text.primary, margin: '0 0 10px', letterSpacing: '-0.02em',
        }}>
          Clinical Demo
        </h1>
